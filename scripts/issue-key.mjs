...
//
// Usage:
//   node scripts/issue-key.mjs --owner "Name" [--contact "..."] [--notes "..."] [--tier standard|partner]
...
if (!values.owner) {
  console.error('usage: node scripts/issue-key.mjs --owner "Name" [--contact "..."] [--notes "..."] [--tier standard|partner]');
  process.exit(1);
}
...
const sql = `INSERT INTO api_keys (key_hash, key_prefix, owner_name, owner_contact, notes, tier, status, created_at) VALUES (${sqlLit(keyHash)}, ${sqlLit(keyPrefix)}, ${sqlLit(values.owner)}, ${sqlLit(values.contact ?? null)}, ${sqlLit(values.notes ?? null)}, ${sqlLit(values.tier)}, 'active', ${now});`;
